...
for(int i = 0; i < MAX_STEPS; i++){\r
    ...
    dist += abs(domain2(loc,u_time));\r
    loc = loc + step_fac*dir*abs(domain2(loc,u_time));\r
  }\r
\r
  float sfac = 4.0;\r
  float rfac = 6.0 * pow((cos(sfac * loc.x) * cos(sfac * loc.y) * cos(sfac * loc.z)),2.0);\r
\r
  vec3 g = grad(loc);\r
  float val = exp(-0.001*pow(dist, 2.2))/1.5;\r
  float val2 = 0.25*g.x+0.75;\r
  \r
  gl_FragColor = vec4(val, val, val, 1.0);\r
\r
 }`;export{r as default};
